...
export const VideoCallTester = () => {
  ...
  const testVideoCall = async () => {
    if (!isVideoConnected || !user) {
      Alert.alert('Error', 'Video service not connected or user not authenticated');
      return;
    }
    ...
    try {
      ...
      if (call) {
        Alert.alert('Success', 'Test call created! Navigating to call screen...');
        router.push({
  ...
  const testAudioCall = async () => {
    ...
    try {
      ...
      if (call) {
        Alert.alert('Success', 'Test audio call created! Navigating to call screen...');
        router.push({
          pathname: '/call/[callId]',
          params: {
            callId: call.id,
  ...
  return (
    <View className="p-4 bg-card rounded-lg m-4">
      <ThemedText className="text-lg font-semibold mb-2">Video Call Testing</ThemedText>
      ...
      <View className="flex-row space-x-2">
        <Button
          ...
          className="flex-1"
        >
          <ThemedText>{isCreatingCall ? 'Creating...' : 'Test Video Call'}</ThemedText>
        </Button>
        ...
        <Button
          ...
          className="flex-1"
        >
          <ThemedText>{isCreatingCall ? 'Creating...' : 'Test Audio Call'}</ThemedText>
        </Button>
      </View>
        {videoClient && (
