...
export const debugCounsellorVerification = async () => {
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      console.log(`  - Will show in list: ${data.isApproved === true && data.verificationStatus === 'verified'}`);
      console.log('---');
    });
...
export const fixExistingCounsellors = async () => {
  try {
    const { collection, getDocs, query, where, doc, updateDoc, writeBatch } = await import('firebase/firestore');
    const { db } = await import('../firebaseConfig');
    ...
    snapshot.forEach((docSnap) => {
      ...
      if (data.verificationStatus === 'approved') {
        ...
        updateCount++;
        console.log(`Fixed ${data.displayName || data.email}: approved -> verified`);
      }
      ...
      if (!data.verificationStatus) {
        ...
        updateCount++;
        console.log(`Fixed ${data.displayName || data.email}: no status -> pending`);
      }
    });
