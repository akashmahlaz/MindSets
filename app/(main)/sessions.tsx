...
import { ActivityIndicator, Alert, RefreshControl, ScrollView, Share, StatusBar, Text, View } from 'react-native';
...
MindConnect Mental Health Platform
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'confirmed': return '#059669'; // Emerald
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'follow-up': return 'checkmark-circle';
      case 'crisis-support': return 'warning';
  ...
  const upcomingSessions = sessions.filter(s => 
    (s.status === 'pending' || s.status === 'confirmed') && s.date > new Date()
  );
    ...
    return (
      <SafeAreaView className="flex-1 bg-background">
        ...
        <View className="flex-1 justify-center items-center">
          ...
          <Text className="text-foreground text-lg mt-4">Loading sessions...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        {/* Header */}
        <View className="py-6">
          <Text className="text-2xl font-bold text-foreground mb-2">My Sessions</Text>
          <Text className="text-muted-foreground">
            {userProfile?.role === 'counsellor' ? 'Manage your client sessions' : 'Track your therapy sessions'}
          </Text>
        </View>
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-1 mr-4">
                <Text className="text-lg font-semibold text-foreground mb-1">
                  {userProfile?.role === 'counsellor' ? 'Schedule New Session' : 'Book New Session'}
                </Text>
                <Text className="text-muted-foreground">
        ...
        {upcomingSessions.length > 0 && (
          <View className="mb-6">
            <Text className="text-xl font-semibold text-foreground mb-4">Upcoming Sessions</Text>
              ...
              <Card key={session.id} className="mb-4 shadow-sm">
                <CardContent className="p-4">
                  ...
                  <View className="flex-row space-x-2">
                    {session.status === 'confirmed' && (
                      ...
                      >
                        <View className="flex-row items-center">
                          ...
                          <Text className="text-primary-foreground text-sm font-medium">Join</Text>
                        </View>
                      </Button>
                    )}
                    ...
                    >
                      <View className="flex-row items-center">
                        <Ionicons name="share" size={16} color={isDarkColorScheme ? "#ffffff" : "#000000"} style={{ marginRight: 4 }} />
        ...
        {completedSessions.length > 0 && (
          <View className="mb-6">
            <Text className="text-xl font-semibold text-foreground mb-4">Recent Sessions</Text>
        ...
        {sessions.length === 0 && (
          <Card className="mt-8 shadow-sm">
            <CardContent className="p-8 items-center">
              <Ionicons name="calendar-outline" size={64} color="#9CA3AF" style={{ marginBottom: 16 }} />
              ...
              <Button onPress={createNewSession} className="px-8">
                <Text className="text-primary-foreground font-medium">
                  {userProfile?.role === 'counsellor' ? 'Schedule Session' : 'Book Session'}
                </Text>
              </Button>
            </CardContent>
          </Card>
