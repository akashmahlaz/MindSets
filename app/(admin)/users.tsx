...
export default function AdminUsers() {
  ...
  const [filter, setFilter] = useState<"all" | "user" | "counsellor" | "admin">(
    "all",
  );
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            if (!newStatus) {
              ...
              Alert.alert("Deactivation Reason", "Please select a reason:", [
                {
                  text: "Violation of terms",
                  onPress: () =>
                    executeStatusChange(
                      userId,
                      newStatus,
                ...
                },
                {
                  text: "Spam/abuse",
                  onPress: () =>
                    executeStatusChange(userId, newStatus, "Spam/abuse"),
                },
                {
                  text: "Inactive account",
                  onPress: () =>
                    executeStatusChange(userId, newStatus, "Inactive account"),
                },
                {
                  text: "User request",
                  onPress: () =>
                    executeStatusChange(userId, newStatus, "User request"),
                },
                { text: "Cancel", style: "cancel" },
              ]);
            } else {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: () => {
            Alert.alert("Deletion Reason", "Please select a reason:", [
              {
                text: "Serious violation",
                onPress: () =>
                  executeDelete(userId, "Serious violation of terms"),
              },
              {
                text: "Fraudulent account",
                onPress: () => executeDelete(userId, "Fraudulent account"),
              },
              {
                text: "Legal request",
                onPress: () => executeDelete(userId, "Legal request"),
              },
              { text: "Cancel", style: "cancel" },
            ]);
          },
              ...
              <Card key={user.uid} className="mb-4">
                <CardHeader>
                  <View className="flex-row justify-between items-start">
                    ...
                    <View className="flex-row space-x-2">
                      <View
                        className={`${getRoleColor(user.role)} border-0 px-2 py-1 rounded-full`}
                      >
                      ...
                      <View
                        className={`${getStatusColor(user.isActive)} border-0 px-2 py-1 rounded-full`}
                      >
                        <Text className="text-white text-xs">
                ...
                <CardContent className="space-y-4">
                    ...
                    <View className="flex-row space-x-2">
                      {["user", "counsellor", "admin"].map((role) => (
                        <Button
                          ...
                          onPress={() =>
                            handleRoleChange(user.uid, role as any)
                          }
                          disabled={
                            user.role === role || processingIds.has(user.uid)
                          }
                          className="flex-1"
                        >
                          <Text
                  ...
                  <View className="flex-row space-x-2">
                    <Button
                      onPress={() =>
                        handleStatusToggle(user.uid, user.isActive !== false)
                      }
                      variant={
