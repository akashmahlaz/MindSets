...
export default function BookSessionScreen() {
  ...
  const [selectedCounselor, setSelectedCounselor] = useState<CounsellorProfileData | null>(null);
  ...
  const loadCounsellors = async () => {
    try {
      ...
      const counsellorList = allUsers.filter(user => user.role === 'counsellor') as CounsellorProfileData[];
  ...
  const bookSession = async () => {
    ...
    try {
      ...
      const sessionType = selectedSessionType.name.toLowerCase().replace(/\s+/g, '-') as 'therapy' | 'consultation' | 'follow-up' | 'crisis-support';
    ...
    return (
      <SafeAreaView className="flex-1 bg-background">
        ...
        <View className="flex-1 justify-center items-center">
          <ActivityIndicator size="large" color={isDarkColorScheme ? "#ffffff" : "#000000"} />
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      <View className="px-6 py-4 border-b border-border flex-row items-center">
        ...
        >
          <Ionicons name="chevron-back" size={24} color={isDarkColorScheme ? "#ffffff" : "#000000"} />
        </TouchableOpacity>
        <View className="flex-1">
          <Text className="text-2xl font-bold text-foreground">Book Session</Text>
      ...
      <ScrollView className="flex-1 px-6 py-4">
        ...
        {step === 1 && (
          <View>
            <Text className="text-xl font-semibold text-foreground mb-4">Choose Your Counselor</Text>
                  ...
                  <View className="flex-row justify-between items-start">
                    <View className="flex-1">
                      ...
                      <Text className="text-muted-foreground mb-2">
                        {counselor.licenseType} • {counselor.yearsExperience} years experience
                      </Text>
                        ...
                        <View className="flex-row flex-wrap gap-1 mb-2">
                          {counselor.specializations.slice(0, 3).map((spec, index) => (
                            <View key={index} className="bg-primary/10 px-2 py-1 rounded">
                              <Text className="text-primary text-xs">{spec}</Text>
                            </View>
                          ))}
                        </View>
                      )}
                    ...
                    <Button onPress={() => selectCounselor(counselor)} className="ml-4">
        ...
        {step === 2 && (
          <View>
            <Text className="text-xl font-semibold text-foreground mb-4">Select Date & Time</Text>
            
            {/* Date Selection */}
            <Text className="text-lg font-medium text-foreground mb-3">Choose Date</Text>
            ...
            {/* Time Selection */}
            <Text className="text-lg font-medium text-foreground mb-3">Choose Time</Text>
        ...
        {step === 3 && (
          <View>
            <Text className="text-xl font-semibold text-foreground mb-4">Session Details</Text>
            
            <Text className="text-lg font-medium text-foreground mb-3">Session Type</Text>
            {sessionTypes.map((type) => (
              ...
              >
                <View className="flex-row justify-between items-start">
                  <View className="flex-1">
                    <Text className="text-lg font-semibold text-foreground mb-1">{type.name}</Text>
                    <Text className="text-muted-foreground mb-2">{type.description}</Text>
                    <Text className="text-sm text-muted-foreground">{type.duration} minutes</Text>
                  </View>
                  <Text className="text-lg font-bold text-foreground">${type.price}</Text>
                </View>
              </TouchableOpacity>
            ))}

            <Text className="text-lg font-medium text-foreground mb-3 mt-4">Additional Notes (Optional)</Text>
            <Input
              value={sessionNotes}
        ...
        {step === 4 && (
          <View>
            <Text className="text-xl font-semibold text-foreground mb-4">Confirm Booking</Text>
              ...
              <CardContent>
                <View className="space-y-3">
                  <View className="flex-row justify-between">
                    ...
                    <Text className="text-foreground font-medium">{selectedCounselor?.displayName}</Text>
                  </View>
                  <View className="flex-row justify-between">
                    <Text className="text-muted-foreground">Date:</Text>
                    <Text className="text-foreground font-medium">{formatDate(selectedDate)}</Text>
                  </View>
                  <View className="flex-row justify-between">
                    <Text className="text-muted-foreground">Time:</Text>
                    <Text className="text-foreground font-medium">{selectedTime?.time}</Text>
                  </View>
                  <View className="flex-row justify-between">
                    <Text className="text-muted-foreground">Session Type:</Text>
                    <Text className="text-foreground font-medium">{selectedSessionType?.name}</Text>
                  </View>
                  <View className="flex-row justify-between">
                    <Text className="text-muted-foreground">Duration:</Text>
                    <Text className="text-foreground font-medium">{selectedSessionType?.duration} minutes</Text>
                  </View>
                  <View className="flex-row justify-between">
                    <Text className="text-muted-foreground">Cost:</Text>
                    <Text className="text-foreground font-bold text-lg">${selectedSessionType?.price}</Text>
                  </View>
                  {sessionNotes && (
                    <View>
              ...
              {booking ? (
                <View className="flex-row items-center">
                  <ActivityIndicator 
                    ...
                    style={{ marginRight: 8 }} 
                  />
                  <Text className="text-primary-foreground font-medium">Booking Session...</Text>
                </View>
              ) : (
                <Text className="text-primary-foreground font-medium">Confirm Booking</Text>
              )}
            </Button>
