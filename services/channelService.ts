...
export class ChannelService {
  ...
  async createDistinctChannel(
    ...
    try {      const channel = this.client.channel(channelType, undefined, {
      ...
      await channel.create();
      console.log(`Distinct channel created for members: ${members.join(', ')}`);
      return channel;
    } catch (error) {
  ...
  async demoteModerators(channel: Channel, moderators: string[]): Promise<void> {
    try {
  ...
  async getUserChannels(
    ...
    try {
      ...
      const channels = await this.client.queryChannels(filter, sort, queryOptions);
