...
export class ChatFeatureTest {
  ...
  async testMemberManagement() {
    ...
    // Add members
    await channelService.addMembers(channel, ["test-user-3"], {
      text: "User 3 joined the channel",
    ...
    // Remove members
    await channelService.removeMembers(channel, ["test-user-3"], {
      text: "User 3 left the channel",
      user_id: this.testUserId,
    });
  ...
  async testDirectMessaging() {
    ...
    const dmChannel = await createOrGetDirectChannel(
      mockUser,
      this.testUserId2,
    );
